...
namespace Lbum.Core.Rss
{
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(LbumRssDefaults.Title, title);
            Description = new XElement(LbumRssDefaults.Description, description);
            Link = new XElement(LbumRssDefaults.Link, link);
            LastBuildDate = new XElement(LbumRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(LbumRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(LbumRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(LbumRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(LbumRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(LbumRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(LbumRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(LbumRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(LbumRssDefaults.Channel,
