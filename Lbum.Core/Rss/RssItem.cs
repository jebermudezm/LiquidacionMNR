...
namespace Lbum.Core.Rss
{
    ...
    public partial class RssItem
    {
        ...
        public RssItem(string title, string content, Uri link, string id, DateTimeOffset pubDate)
        {
            ...
            Content = new XElement(LbumRssDefaults.Description, content);
            Link = new XElement(LbumRssDefaults.Link, link);
            Id = new XElement(LbumRssDefaults.Guid, new XAttribute("isPermaLink", false), id);
            PubDate = new XElement(LbumRssDefaults.PubDate, pubDate.ToString("r"));
        }
        ...
        {
            var title = item.Element(LbumRssDefaults.Title)?.Value ?? string.Empty;
            var content = item.Element(LbumRssDefaults.Content)?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element(LbumRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(item.Element(LbumRssDefaults.Link)?.Value ?? string.Empty);
            var pubDateValue = item.Element(LbumRssDefaults.PubDate)?.Value;
            var pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            var id = item.Element(LbumRssDefaults.Guid)?.Value ?? string.Empty;
        ...
        {
            var element = new XElement(LbumRssDefaults.Item, Id, Link, Title, Content);
