...
using Lbum.Core.ComponentModel;
using Lbum.Core.Configuration;
using Lbum.Core.Redis;
using Lbum.Core.Security;
using StackExchange.Redis;

namespace Lbum.Core.Caching
{
    /// <summary>
    ...
    public partial class RedisCacheManager : IStaticCacheManager
    {
        ...
        private readonly LbumConfig _config;
        ...
        protected virtual IEnumerable<RedisKey> GetKeys(EndPoint endPoint, string prefix = null)
        {
            ...
            keys = keys.Where(key => !key.ToString().Equals(LbumDataProtectionDefaults.RedisDataProtectionKey,
        ...
        {
            //we should always persist the data protection key list
            if (key.Key.Equals(LbumDataProtectionDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase))
                return;
